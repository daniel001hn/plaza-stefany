...
// This allows PlazaStefany.jsx to keep using window.storage.get/set without changes
...
const storage = {
    ...
    async set(key, value) {
          ...
          const { error } = await supabase.from('kv_store').upsert({ key, value: parsed, updated_at: new Date().toISOString() }, { onConflict: 'key' })
